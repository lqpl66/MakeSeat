...
public class PushMessageForLine {
	...
	// JsonObject obj = new
	// JsonParser().parse(httpOrgCreateTestRtn.substring(16)).getAsJsonObject();
	...
	// if (list.size() == 0) {
	// JsonObject obj1 = new
	// JsonParser().parse(pair.getValue().toString()).getAsJsonObject();
	// for (Entry<String, JsonElement> pair1 : obj1.entrySet()) {
	// JsonArray js = null;
	// if (pair1.getValue().isJsonArray()) {
	// if (pair1.getKey().contains("Z") || pair1.getKey().contains("T") ||
	// pair1.getKey().contains("K")
	// || pair1.getKey().contains("D")) {
	// js = pair1.getValue().getAsJsonArray();
	// for (int i = 0; i < js.size(); i++) {
	// String station_train_code =
	// js.get(i).getAsJsonObject().get("station_train_code")
	// .getAsString();
	// String train_no =
	// js.get(i).getAsJsonObject().get("train_no").getAsString();
	// String fromName =
	// station_train_code.substring(station_train_code.indexOf("(") + 1,
	...
	public void doIt() {
		...
		stationOrderService.orderBuyClose();
	}
}
